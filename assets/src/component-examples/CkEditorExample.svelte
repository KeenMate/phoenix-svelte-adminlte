<script>
    ...
    let data = {
      name: "CkEditor",
      text: "CkEditor from https://ckeditor.com/ wraped inside svelte component for easier use.If you want to bind it contents of editor, cant just bind value to your variable and insted you have to use input event with combination to value. It uses html with style to format.",
      props: [
      ...
      examples: {
        basic: {
          name: "CkEditor usage",
          code: '<CkEditor bind:value={startingValue} on:input={(e) => {notification.success(e.detail,\"CkEditor data\")}}>\r\n<\/CkEditor>',
        }
  ...
  >
    <svelte:fragment slot="examples">
      ...
      >
        <CkEditor bind:value={startingValue} on:input={(e) => {notification.success(e.detail.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;"),"CkEditor event e.datail")}}>
        </CkEditor>
      </ComponentExampleTemplate>
