...
const azureConfiguration = {
	authority:
		"https://login.microsoftonline.com/6ee623a2-0b05-4ea4-b931-79c555955cb1/v2.0/",
...
export const provider = writable(
	localStorage.getItem(LocalStorageProviderKey) || null
);
...
export const userManager = derived(provider, ($provider) =>
	getUserManager($provider)
);
...
function userLoadedCallback(user) {
	...
	console.log(user);
	saveToCookie(user.access_token);
}
...
const acCookieSettings = { sameSite: "strict" };

function saveToCookie(ac) {
	Cookies.set("accessToken", ac, acCookieSettings);
}
function deleteCookie() {
	Cookies.remove("accessToken", acCookieSettings);
}
