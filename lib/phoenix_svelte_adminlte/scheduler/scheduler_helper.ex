defmodule PhoenixSvelteAdminlte.Scheduler.SchedulerHelper do
  ...
  def create_job(name, func, cron_expression) when is_bitstring(cron_expression) do
    case Crontab.CronExpression.Parser.parse(cron_expression) do
      {:ok, parsed_cron} ->
        IO.inspect(parsed_cron, label: "cron")
  ...
  defp tmp_get_script(script_id) do
    case script_id do
      1 ->
        ~s(
				DELETE
  ...
  def create_db_job(name, script_id, cron_expression) do
    create_job(
      name,
      {PhoenixSvelteAdminlte.Scheduler.SchedulerHelper, :run_database_script, [script_id]},
      cron_expression
    )
  end
end
